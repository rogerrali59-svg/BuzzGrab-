...
import { adminBrandList, adminDeleteBrand, adminUpdateStatusBrand } from "../../../services/services";
...
const BrandList = () => {
  ...
  usePageTitle(`${constant.PROJECT_NAME} | Brand List`);
  ...
  const {
    ...
    queryKey: ["brand-list", page, debouncedSearch, state],
    queryFn: async () => {
      const resp = await adminBrandList(page, debouncedSearch, state);
  ...
  const { mutate } = useMutation({
    mutationFn: (payload) => adminDeleteBrand(payload),
    onSuccess: (resp) => {
  ...
  const statusMutation = useMutation({
    mutationFn: (payload) => {
      return adminUpdateStatusBrand(payload.id, payload.stateId);
    },
    onSuccess: (resp) => {
      toastAlert("success", resp?.data?.message)
      queryClient.invalidateQueries({ queryKey: ["brand-list"] });
