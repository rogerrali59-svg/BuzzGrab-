...
import { adminDeleteProduct, adminProductList, adminUpdateProductStatus } from "../../../services/services";
import { ProductStatus } from "../../../utils/CheckAdminState";
import { constant, Paginations } from "../../../utils/constant";
import { formatCurrency, serialNumber, truncate } from "../../../utils/helper";
...
const ProductList = () => {
  ...
  usePageTitle(`${constant.PROJECT_NAME} | Store`);
  ...
  const {
    ...
    queryKey: ["product-list", page, debouncedSearch, state],
    queryFn: async () => {
      const resp = await adminProductList(page, debouncedSearch, state);
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (payload) => adminDeleteProduct(payload),
    onSuccess: (resp) => {
  ...
  const statusMutation = useMutation({
    mutationFn: (payload) => {
      return adminUpdateProductStatus(payload.id, payload.stateId);
    },
    onSuccess: (resp) => {
      toastAlert("success", resp?.data?.message)
      queryClient.invalidateQueries({ queryKey: ["product-list"] });
                      ...
                      productListing?.map((data, index) => {
                        return (
                          <tr key={index}>
                              ...
                              <div className="table-user d-flex align-items-center">
                                <span className="table-user-icon">
                                  <img
                                    ...
                                    src={data?.productImg?.at(0)?.url || "/images/static_image.jpg"}
                                    alt="product-img"
                            ...
                            <td className="text-capitalize">{truncate(data?.productName, 50)}</td>
                            <td>{data?.size}</td>
                            ...
                            <td>{ProductStatus(data?.stateId)}</td>
                            <td>
                              <div className="action-btn">
                                <button
                                  title="View"
                                  onClick={() => { navigate(`/admin/view-product/${data?._id}`) }}
                                ...
                                <button
                                  ...
                                  title="Update"
                                  onClick={() => { navigate(`/admin/edit-product/${data?._id}`) }}
                                >
                                  <FaPencilAlt />
                                </button>
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    statusMutation.mutate({
                                      id: data?._id,
                                      stateId:
                                        data?.stateId === constant.PRODUCT_INACTIVE || data?.stateId === constant.PRODUCT_DELETED
                                          ? constant.PRODUCT_ACTIVE
                                          : constant.PRODUCT_INACTIVE,
                                    });
                                  }}
                                  className={`btn-small ${data?.stateId === constant.PRODUCT_ACTIVE
                                    ? "btn btn-small btn-warning"
                                    : "btn btn-small btn-success"
                                    }`}
                                >
                                  {data?.stateId === constant.PRODUCT_ACTIVE ? (
                                ...
                                <button
                                  title="Delete"
                                  disabled={data?.stateId === constant.PRODUCT_DELETED}
