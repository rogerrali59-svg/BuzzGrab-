...
const DriverList = () => {
    ...
    usePageTitle(`${constant.PROJECT_NAME} | Registered Drivers`);
    ...
    const {
        ...
        queryKey: ["driver-List", page, debouncedSearch, state, constant.DRIVER_ROLE],
        queryFn: async () => {
            const resp = await adminUserList(page, debouncedSearch, state, constant.DRIVER_ROLE);
            setMeta(resp?.data?._meta);
            return resp?.data?.data ?? [];
        },
    });

    // Mutation for driver update
    const stateMutation = useMutation({
        mutationFn: ({ id, body }) => adminUserUpdate(id, body),
        onSuccess: (resp) => {
            if (resp?.data?.data?.stateId == constant.DELETED) {
                toastAlert("success", "Driver account has been deleted successfully")
            }
                                                ...
                                                <tr key={index}>
                                                    ...
                                                    <td>
                                                        <div className="action-btn">
                                                            ...
                                                            <button
                                                                title="View"
                                                                onClick={() => { navigate(`/admin/view-driver/${data?._id}`) }}
                                                            ...
                                                            <button
                                                                ...
                                                                title="Update"
                                                                onClick={() =>
                                                                    navigate(`/admin/edit-driver/${data?._id}`)
                                                                }
                                                            >
                                                                <FaPencilAlt />
                                                            </button>
            ...
            >
                <Form onSubmit={handleSubmit}>
                    <Modal.Header closeButton>
                        <Modal.Title>
                            {stateModal.actionType === "delete"
                                ? " Reason for Deleting Driver Account"
                                : " Reason for In-Active Driver Account"}
                        </Modal.Title>
                    </Modal.Header>
