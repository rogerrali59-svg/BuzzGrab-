...
const UserList = () => {
  ...
  usePageTitle(`${constant.PROJECT_NAME} | Registers User`);
  ...
  const {
    ...
    queryKey: ["user-List", page, debouncedSearch, state, constant.USER_ROLE],
    queryFn: async () => {
      const resp = await adminUserList(page, debouncedSearch, state, constant.USER_ROLE);
  ...
  const stateMutation = useMutation({
    ...
    onSuccess: (resp) => {
      if (resp?.data?.data?.stateId == constant.DELETED) {
        toastAlert("success", "User account has been deleted successfully")
      }
      ...
      >
        <Form onSubmit={handleSubmit}>
          <Modal.Header closeButton>
            <Modal.Title>
              {stateModal.actionType === "delete"
                ? " Reason for Deleting User Account"
