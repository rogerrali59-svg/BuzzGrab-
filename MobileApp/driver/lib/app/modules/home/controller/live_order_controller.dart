import 'package:alcoholdeliverydriver/app/data/remote_service/network/network_exceptions.dart';
...
loadStaticData();
  ...
  void nextStep() {
    debugPrint('currentStep.value ${currentStep.value}');

    if (currentStep.value < 4) {
      // 4 total steps
      currentStep.value= currentStep.value+1;
      debugPrint('currentStep.value now ${currentStep.value}');
      if (currentStep.value == 0) {
        //  commented — will restore later when api provided
  ...
  markCompleteOrder() async {
    if (otpController.text.isEmpty) return toast("Enter OTP");

    // API commented — will restore later
    /*
  await DioClient().post('/api/mark-order-complete-api/', ... );
  */

    currentStep.value = 4;         // completed
    getOrderDetails();             // refresh UI
  ...
  changeOrderStatus({choice, orderId}) async {
    ...
    await DioClient()
      ...
      getOrderDetails();
      currentStep.value++;
      //commented for later use when api provided
  ...
  void loadStaticData() {
    ...
    orderDetailsModel.value = getStaticOrderDetails();
    update();
  }
  ...
  updateStatus() {
    debugPrint( 'orderDetailsModel.value.data?.status ${orderDetailsModel.value.data?.status}');
