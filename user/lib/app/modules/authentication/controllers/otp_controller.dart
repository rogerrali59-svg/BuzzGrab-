...
class OtpController extends TbaseController {
  ...
  hitUserResendOtp() async {
    ...
    try {
      ...
      customLoader.hide();
      Future.error(
          NetworkExceptions.getDioException(e, str, "auth/resendOtp"));
      customLoader.hide();
    ...
    try {
      ...
      if (isForgot.value == true) {
        Get.offNamed(AppRoutes.changePassword,
            arguments: {'isForgot': true, 'email': email});
      } else {
        Get.toNamed(AppRoutes.permissionScreen);
      }
      customLoader.hide();
    } catch (e, str) {
      customLoader.hide();
      Future.error(
          NetworkExceptions.getDioException(e, str, "auth/verifyOtp"));
      showInSnackBar(message: NetworkExceptions.messageData);
    }
  }
}
